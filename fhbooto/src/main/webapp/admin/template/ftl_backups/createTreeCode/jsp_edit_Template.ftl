...
<body data-sa-theme="${r"${sessionScope.SKIN}"}">
	...
	<div class="card-body">
		 ...
		 <form action="${objectNameLower}/${r"${msg }"}" name="Form" id="Form" method="post">
			...
			<h4 class="card-title">上级：${r"${null == pds.NAME ?'(无) 此为顶级':pds.NAME}"}</h4>
			
			<div class="input-group" style="margin-top:3px;">
                <span class="input-group-addon" style="width: 79px;"><span style="width: 100%;">名称</span></span>
                <div class="form-group">
                    <input type="text" class="form-control" name="NAME" id="NAME" value="${r"${pd."}NAME${r"}"}"  placeholder="这里输入名称" title="名称">
