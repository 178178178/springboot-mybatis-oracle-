...
<body style="background-color: rgba(255,255,255,0);">
				...
				<c:choose>
					<c:when test="${r"${not empty varList}"}">
						<c:forEach items="${r"${varList}"}" var="var" varStatus="vs">
							<tr>
								<td>
									<input type="checkbox" style="margin-top: 2px;" name='ids' value="${r"${var."}${objectNameUpper}_ID${r"}"}">
								</td>
			...
			<table style="width:100%;margin-top:15px;">
				<tr>
					<td style="vertical-align:top;">
						<shiro:hasPermission name="${objectNameLower}:add"><a class="btn btn-light btn--icon-text" onclick="add();">新增</a></shiro:hasPermission>
						<shiro:hasPermission name="${objectNameLower}:del"><a class="btn btn-light btn--icon-text" onclick="makeAll('确定要删除选中的数据吗?');">删除</a></shiro:hasPermission>
					</td>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>${objectNameLower}/goAdd';
			 diag.Width = 600;
			 diag.Height = 500;
			...
			swal({
    			...
    			if(str==''){
    				...
    				if(msg == '确定要删除选中的数据吗?'){
    					$.ajax({
    						type: "POST",
    						url: '<%=basePath%>${objectNameLower}/deleteAll?tm='+new Date().getTime(),
    				    	data: {DATA_IDS:str},
    						dataType:'json',
    						cache: false,
    						success: function(data){
