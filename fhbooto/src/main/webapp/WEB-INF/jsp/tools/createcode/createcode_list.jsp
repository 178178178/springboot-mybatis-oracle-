...
<body style="background-color: rgba(255,255,255,0);">
      ...
      <div class="card" style="margin-top:5px;">
         <div class="card-body">
          <form action="createCode/list" method="post" name="Form" id="Form">
			<!-- 检索条件  -->
			<table>
				<tr>
					<td>
				 ...
				 <c:choose>
					<c:when test="${not empty varList}">
                 	 <c:forEach items="${varList}" var="var" varStatus="vs">
		                 <tr>
							 <td>
							  	<input type="checkbox" style="margin-top: 2px;" name='ids' value="${var.CREATECODE_ID}">
							 </td>
		                     <td>${page.showCount*(page.currentPage-1)+vs.index+1}</td>
		                     <td>${var.TITLE}</td>
							 <td>${var.PACKAGENAME}</td>
							 <td>${var.OBJECTNAME}</td>
							 <td>${fn:replace(var.TABLENAME, ',fh,', '')}</td>
							 <td>
		                     ...
		                     <td>
		                     	<shiro:hasPermission name="fhbutton:edit"><a title="编辑" onclick="productCode('${var.CREATECODE_ID}');"><i class="zmdi zmdi-edit zmdi-hc-fw"></i></a></shiro:hasPermission>
		                     	<shiro:hasPermission name="fhbutton:del"><a title="删除" onclick="del('${var.CREATECODE_ID}');"><i class="zmdi zmdi-close zmdi-hc-fw"></i></a></shiro:hasPermission>
		                     </td>
		                 </tr>
	                 </c:forEach>
	                </c:when>
             ...
             <table style="width:100%;margin-top:15px;">
				<tr>
					<td style="vertical-align:top;">
						<shiro:hasPermission name="fhbutton:add"><a class="btn btn-light btn--icon-text" onclick="productCode('add');">启动生成器</a></shiro:hasPermission>
	...
	<script type="text/javascript">
		...
		function productCode(CREATECODE_ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="代码生成器";
			 diag.URL = '<%=basePath%>createCode/goProductCode?CREATECODE_ID='+CREATECODE_ID;
			 diag.Width = 900;
			 diag.Height = 600;
		...
		function del(Id){
			swal({
            	...
            	$.ajax({
        			type: "POST",
        			url: '<%=basePath%>createCode/delete',
			...
			swal({
    			...
    			if(str==''){
    				...
    				if(msg == '确定要删除选中的数据吗?'){
    					$.ajax({
    						type: "POST",
    						url: '<%=basePath%>createCode/deleteAll?tm='+new Date().getTime(),
    				    	data: {DATA_IDS:str},
    						dataType:'json',
    						cache: false,
    						success: function(data){
