...
<body style="background-color: rgba(255,255,255,0);">
				 ...
				 <c:choose>
					<c:when test="${not empty varList}">
                 	 <c:forEach items="${varList}" var="var" varStatus="vs">
		                 <tr>
							 <td>
							  	<input type="checkbox" style="margin-top: 2px;" name='ids' value="${var.FHBUTTON_ID}">
							 </td>
		                     ...
		                     <td>${var.BZ }</td>
		                     <td>
		                     	<shiro:hasPermission name="fhbutton:edit"><a title="修改" onclick="edit('${var.FHBUTTON_ID}');"><i class="zmdi zmdi-edit zmdi-hc-fw"></i></a></shiro:hasPermission>
		                     	<shiro:hasPermission name="fhbutton:del"><a title="删除" onclick="del('${var.FHBUTTON_ID }');"><i class="zmdi zmdi-close zmdi-hc-fw"></i></a></shiro:hasPermission>
		                     </td>
		                 </tr>
	                 </c:forEach>
	                </c:when>
             ...
             <table style="width:100%;margin-top:15px;">
				<tr>
					<td style="vertical-align:top;">
						<shiro:hasPermission name="fhbutton:add"><a class="btn btn-light btn--icon-text" onclick="add();">新增</a></shiro:hasPermission>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>fhbutton/goAdd';
			 diag.Width = 600;
			 diag.Height = 325;
			...
			swal({
    			...
    			if(str==''){
    				...
    				if(msg == '确定要删除选中的数据吗?'){
    					$.ajax({
    						type: "POST",
    						url: '<%=basePath%>fhbutton/deleteAll?tm='+new Date().getTime(),
    				    	data: {DATA_IDS:str},
    						dataType:'json',
    						cache: false,
    						success: function(data){
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>fhbutton/goEdit?FHBUTTON_ID='+Id;
			 diag.Width = 600;
			 diag.Height = 325;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
